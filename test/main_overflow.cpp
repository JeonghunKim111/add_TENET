	...
	const char* _mapping_file)
{
	...
	double ingress_delay = df.GetIngressDelay(isl_union_map_copy(space_time_to_neighbor));
	double egress_delay = df.GetEgressDelay(isl_union_map_copy(space_time_to_neighbor));
	double computation_delay = df.GetComputationDelay();
	fprintf(stdout, "Delay: In: %f; Out: %f; Com: %f\n", ingress_delay, egress_delay, computation_delay);

	double dsize = df.GetActivePENum();
	double avg_dsize = df.GetAverageActivePENum();
	fprintf(stdout, "Active PE Num: %f; Average: %.2f\n", dsize, avg_dsize);
